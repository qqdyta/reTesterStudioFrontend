...
const handleEdit = (index, row) => {
  ...
  fetch(proxy.$serverUrl + 'testPlan/', {
    ...
    body: JSON.stringify({typed: 'getSingleTestPlan', id: row.id}), // body data type must match "Content-Type" header
  }).then(response => response.json())
      .then(data => {
        const RESULT = data.data[0]
        console.log('Success:', RESULT);
        proxy.$emitter.emit('settingRightBox', ['add'])
        setTimeout(()=>{
          proxy.$emitter.emit('loadTestStep', {'testStep': RESULT.testStep, 'edges': RESULT.testEdge, id: RESULT.id })
        }, 200)
      })
      .catch((error) => {
